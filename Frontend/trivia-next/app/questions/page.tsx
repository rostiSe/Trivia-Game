...
export default function QuestionsPage() {
  ...
  async function deleteQuestion(id: string) {
    ...
    async function saveEditedQuestion() {
      ...
      const newIncorrect = editingQuestion.answers.filter((a) => a !== newCorrect);
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/questions/update`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        setQuestions((prev) =>
          prev.map((q) => (q.id === editingQuestion.id ? { ...q, ...updatedPayload } : q))
        );
  ...
  return (
    ...
    <div className="text-center mb-10">
      ...
      <p className="text-xl text-indigo-200">Review or edit your saved questions</p>
    </div>
          ...
          <Card key={question.id}>
            <div className="w-full flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                >
                  {isDeleting ? <Trash2Icon className="text-red-500 transition-all group-hover:text-indigo-200 size-5"/> : <Trash className="text-red-500 transition-all group-hover:text-pink-400 size-5" />}
                </Button>
              </div>
            </div>
            ...
            {isEditing ? (
              ...
              <div className="mt-4">
                <h1 className="text-xl pb-5 font-medium">{decode(question.question)}</h1>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
